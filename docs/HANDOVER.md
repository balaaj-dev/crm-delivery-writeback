...
| ~~Smartlead webhook-registration endpoint + payload shape~~ | `lib/sources/smartlead-api.ts` `registerSmartleadWebhook` | **CONFIRMED LIVE 26 Aug 2026** against a real Lotus Labs campaign: `POST /campaigns/{id}/webhooks` with `{name, event_types, webhook_url}` returns `200 {ok, id, event_type_map, ...}`. The real gap wasn't the path — it was a required `name` field the brief never mentioned; every registration attempt in a real wizard run failed with a 400 (`"name" is required`) until this was added. Verified event_type_map only enables the 7 approved events, never the forbidden open/click ones. |
