...
const PENDING_WRITEBACK_FIELDS: Record<string, string | null> = {
  ...
  crmCredentials: null, // 🔁 CRM Credentials (generic JSON blob — fallback for CRM types with no dedicated field below)
  hubspotServiceKey: null, // 🔑 HubSpot Service Key — plain-text token, not JSON; see mapAirtableRecordToClientConfig
  ...
  dealStageOnCreate: null, // 🔁 Deal Stage On Create (legacy — see dealStageOnPositiveReply/MeetingBooked)
  dealStageOnPositiveReply: null, // 🔁 Deal Stage On Positive Reply
  dealStageOnMeetingBooked: null, // 🔁 Deal Stage On Meeting Booked
  planLimitAcknowledged: null, // 🔁 Plan Limit Acknowledged
  ownerId: null, // 🔁 Owner ID — required guardrail, see lib/types.ts's ClientConfig.behaviour.ownerId
  campaignIds: null, // 🔁 Campaign IDs — comma-separated Smartlead campaign IDs; empty = all campaigns
  webhookSecret: null, // 🔁 Webhook Secret — generated by the app at registration time, not hand-entered
  smartleadWebhookId: null, // 🔁 Smartlead Webhook ID — generated by the app at registration time, not hand-entered
};
...
export function mapAirtableRecordToClientConfig(record: AirtableRecord): ClientConfig | null {
  ...
  // HubSpot's credential is a single plain-text token — asking a CSM to
  // hand-write `{"accessToken": "..."}` into a generic JSON field is a real
  // source of malformed-JSON errors, so it gets its own plain field instead.
  // Falls back to the generic crmCredentials JSON blob (needed for any
  // future CRM type whose credentials aren't a single token). If both are
  // somehow set, the dedicated field wins.
  const hubspotServiceKeyRaw = get('hubspotServiceKey');
  const credentials =
    crmType === 'hubspot' && typeof hubspotServiceKeyRaw === 'string' && hubspotServiceKeyRaw.trim() !== ''
      ? { accessToken: hubspotServiceKeyRaw.trim() }
      : safeParseJson(get('crmCredentials'), credentialsSchema, {});

  const campaignIdsRaw = get('campaignIds');
  const campaignIds =
    typeof campaignIdsRaw === 'string' && campaignIdsRaw.trim() !== ''
      ? campaignIdsRaw.split(',').map((id) => id.trim()).filter(Boolean)
      : undefined;

  const smartleadWebhookIdRaw = get('smartleadWebhookId');
  const smartleadWebhookId =
    typeof smartleadWebhookIdRaw === 'number'
      ? smartleadWebhookIdRaw
      : typeof smartleadWebhookIdRaw === 'string' && smartleadWebhookIdRaw.trim() !== ''
        ? Number(smartleadWebhookIdRaw)
        : undefined;
  ...
  return {
    ...
    source: {
      ...
      campaignIds,
      webhookSecret: get('webhookSecret') as string | undefined,
      smartleadWebhookId,
    },
    crm: {
      type: crmType,
    ...
    behaviour: {
      ...
      dealStageOnPositiveReply: get('dealStageOnPositiveReply') as string | undefined,
      dealStageOnMeetingBooked: get('dealStageOnMeetingBooked') as string | undefined,
      planLimitAcknowledged: yesNo(get('planLimitAcknowledged')),
      ownerId: get('ownerId') as string | undefined,
    },
    events: {
      email_sent: false,
      reply: false,
